/**
 * Renderiza componente do console dentro do `node --test`.
 *
 ...
 *
 *     import { criar, textoVisivel } from "./ferramentas/renderizar.mjs";
 *     const { FichaDoAssinante } = await import("../src/components/FichaDoAssinante.jsx");
 *     const texto = textoVisivel(criar(FichaDoAssinante, { ficha }));
 *
 * O teste é escrito em `.mjs` puro, sem JSX, e monta o elemento com `criar`
 * (o `createElement` do React). Um arquivo de teste em JSX precisaria dos hooks
 * para carregar a si mesmo, que é o ovo antes da galinha.
 *
 * `renderToStaticMarkup` não roda `useEffect`, de propósito: o que se testa
 * aqui é o DESENHO a partir das props, e componente do console recebe dado por
 * prop, porque quem busca é o `App`.
 */
